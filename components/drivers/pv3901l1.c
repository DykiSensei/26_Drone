...
static void parse_byte(uint8_t byte)
{
    ...
    case 2: /* 接收数据 */
        ...
        if (data_cnt >= 9) {
            ...
            if (sum_ok && tail_ok) {
                /* 模块原始轴 → 机体系映射。机体系约定：flow_x=前正、flow_y=右正
                 * —— 下游陀螺补偿配对/米制换算/控制符号全部依赖此约定。
                 * 2026-07-07 换装重验（手持台架实测）：前移→raw_y 负、右移→raw_x 正，
                 * 故 flow_x=−raw_y、flow_y=+raw_x（较 2026-07-04 装向转了 180°）。
                 * 换装或旋转模块后只改这两行，并重验陀螺补偿符号(kx/ky)。 */
                int16_t raw_x = (int16_t)((rx_buf[3] << 8) | rx_buf[2]);
                int16_t raw_y = (int16_t)((rx_buf[5] << 8) | rx_buf[4]);
                portENTER_CRITICAL(&g_lock);
                g_flow.flow_x = (int16_t)(-raw_y);  /* 机体前正 = 模块 −y */
                g_flow.flow_y = raw_x;              /* 机体右正 = 模块 +x */
