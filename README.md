...
⚠️ 抓取任务要求处于定高/定点模式；切出模式、油门归零或 TOF 失效会自动中止。任务完成后爪保持闭合，用"机械爪→张开"按钮投放。
...
{"cmd": "grab_abort"}                     // 中止抓取任务（回起始高度，爪保持当前角）
{"cmd": "grab_cfg", "tof": 0.20}          // 抓取触发高度调试：TOF 读数低于此闭爪（默认 0.20 = TOF 落地安装读数）
```
