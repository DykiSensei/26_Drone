...
| 机械爪舵机 | MG995 | LEDC PWM | GPIO10, 50Hz。实测 0°=张开 / 90°=闭合，**>90° 齿条空转，固件硬限位 90°**；爪兼作起落架，上电默认张开。⚠️ 必须独立 5V BEC 供电（堵转电流 1A+，与逻辑电共轨会在闭爪瞬间拉复位主控） |
...
{"cmd": "grip_calib", "on": 1}            // 舵机标定模式：仅锁定态生效，滑条限位放开到 0-180 探索开/合角，慢速 45°/s；解锁/断连自动恢复 90° 限位
...
- [x] MG995 机械爪驱动 + 台架标定（0°张开/90°闭合、90° 硬限位、爪兼起落架；WS `grip` 命令 + 前端面板，2026-07-05）
...
- [ ] 机械爪开/合角重标定（用"舵机标定"模式全量程探索，回填 servo_grip.h 三宏）
