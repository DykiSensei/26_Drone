...
| GPS+磁力计 | BN-880（已购，**待接入**） | 磁力计 I2C / GPS UART | 磁力计并入 I2C0；芯片可能是 HMC5883L(0x1E) 或 QMC5883L(0x0D) |
...
| 位置偏移 | `{"cmd": "move_to", "x": 0.5, "y": -0.3}` | P4 视觉定位，偏移单位**米**（钳位 ±3），航位推算闭环 |
...
```json
{
  ...
  "flow": {
    ...
    "cx": 0.0, "cy": 0.0,       // 补偿+平滑后的光流速度 (m/s)
    "ps": 0,                    // 位置环状态 0=idle 1=pending 2=hold 3=move_to
    "tx": 0, "ty": 0,           // 位置锁定目标（米）
    "fc": 0, "ec": 0,           // 光流帧计数 / 校验错误计数
    "fx": 0, "fy": 0,           // 原始 flow_x/y（本帧，counts）
    "vx": 0.0, "vy": 0.0        // IMU/flow 互补滤波速度估计 (m/s)
  },
...
- [ ] **米制化 + 安全加固上机试飞验证**（悬停定点精度 + 垂直起飞）
- [ ] BN-880 磁力计接入（I2C0 共享总线；Mahony 九轴 yaw 修正 → 光流世界系积分）
