...
- `servo_grip.h` — `servo_grip_init()` (GPIO10, LEDC TIMER_1/CH4 @50Hz, 500–2500µs↔0–180°; **non-fatal** on failure), `servo_grip_set_angle()` (**驱动层硬限位 `SERVO_GRIP_MAX_DEG=117°` — 齿条行程极限, 超过空转, 标定实测 2026-07-06**), `servo_grip_open()`/`servo_grip_close()` (presets `SERVO_GRIP_OPEN_DEG=0`/`SERVO_GRIP_CLOSE_DEG=110` [行程极限−7° 防堵转], 实测值; 爪兼作起落架, 上电默认张开), `servo_grip_update(dt)` (100Hz slew, 120°/s — 防电流尖峰/飞行反扭), `servo_grip_set_calib_mode()` (标定模式: 限位放开 0–180/慢速 45°/s, main.c 每拍门控仅 DISARMED), `servo_grip_get_angle()`, `servo_grip_is_moving()`
