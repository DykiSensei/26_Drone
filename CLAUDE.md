...
1d. **Gyro high-pass for the comp term (2026-07-04)** — dead-reckoned position ran meters-per-minute at rest even at 1m height with good qual: gyro DC bias (temp drift / post-handling shift) injects a constant fake velocity `k×bias×h` through the metric comp term — 1°/s bias at 1m = 0.017 m/s, past the deadband; scales with height. Fix: comp uses only the high-passed gyro (`ω − ω_lp`, `GYRO_LP_ALPHA` τ≈1-2s); the bias-estimate LP states survive `flow_hold_reset` so takeoff comp is immediately effective. Real hover rotation is zero-mean/transient, so the HPF costs nothing. Verify at rest: telemetry `Vel vx/vy` ≈ 0.00 and X/Y static.
