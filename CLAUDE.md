...
1b. **Flow mount remap (2026-07-04)** — module found mounted 90° rotated (forward→raw_y+, left→raw_x+); remapped to body frame in `pv3901l1.c parse_byte()`. ⚠️ Gyro-comp gains kx/ky (−2.5 defaults) were calibrated pre-remap — **re-verify with the tilt test** (tilt slowly without translating; adjust kx/ky until telemetry `Comp x/y` ≈ 0; sign may have flipped).
