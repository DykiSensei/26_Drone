...
⚠️ **NO-FLY GATE — root cause found 2026-07-07, fix applied, awaiting re-flash + re-verify**: the optical-flow regression had **two causes**, both from the 2026-07-06 remount: (1) the drone frame/claw intruded into the sensor FOV — a static textured object anchors PMW3901 flow to ~0 *with high qual*, which is why "high qual" did NOT rule out obstruction; hand-carry then under-reported asymmetrically. Fixed by remounting clear of the FOV (user, 2026-07-07). (2) The module ended up 180° from its 2026-07-04 orientation → both flow axes sign-inverted (forward read `flow.x`≈−0.8, right read `flow.y`≈−1.2). Fixed in `pv3901l1.c parse_byte()` (`flow_x=-raw_y`, `flow_y=raw_x`). **STILL ACTIVE until the user flashes this build and confirms: forward→`flow.x`>0, right→`flow.y`>0, cross-terms ≈0, the gyro-comp tilt test still nulls (kx/ky), and scale re-measured (~0.8/1.2 seen, likely hand-carry noise — average ~1.0).** An axis error makes the position loop a positive-feedback fly-away. Do not plan flight until this re-verification passes.
...
**Current priority — shrink the ~30cm hover drift** (all-flow levers, no new hardware; blocked by the no-fly gate above):
