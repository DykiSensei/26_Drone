...
**Flight-verified**: STABILIZE, ALT_HOLD (vz damping + takeoff target ramp), auto-takeoff, per-motor trim. (The old counts-domain gyro-comp calibration kx=ky=−2.5 is obsolete — comp is metric-domain now, see 1b.)

**Done but NOT yet flight-tested** (builds clean for esp32s3):
1. **Metric flow refactor** — all horizontal control now in m/s / meters (details in Architecture below). Motivation: raw flow-unit gains varied 5–10× with altitude → hover instability + lateral drift during takeoff climb. ⚠️ Before trusting position hold, calibrate `flow_scale` (default 0.00244 rad/count) using the web 标定模式 button (`flow_calib` — safe: DISARMED, motors stopped, props on): hand-carry at a fixed height over textured floor, move a known distance (e.g. 1 m), compare telemetry `X (m)`, then `new_scale = 0.00244 × actual / displayed` via the FlowScale input. Value is lost on reboot (no NVS yet).
1b. **Flow mount remap + metric gyro comp (2026-07-04)** — module found mounted 90° rotated (forward→raw_y+, left→raw_x+); remapped to body frame in `pv3901l1.c parse_byte()`. Gyro comp moved from counts domain to metric domain (comp = k × ω × height; counts-domain constant k could not null once dt_frame became measured/jittering — tilt test swung at every k value). kx/ky are dimensionless, nominal ±1.0; **default +1.0/+1.0 (bench 2026-07-04: +1 clearly smaller swing than −1)**. Tilt-test acceptance: residual swing will NOT reach 0 in a hand test (wrist rotation axis ≠ camera nodal point → real lever-arm translation; TOF slant jitter). Judge by comparing against k=0: ≥~70% swing reduction = calibrated. Test each axis independently (pitch-rock → kx via `Comp x`, roll-rock → ky via `Comp y`) — the two signs need not match.
1c. **Flow-invalid freeze (2026-07-04)** — when `quality_gain < FLOW_MIN_TRUST (0.05)` or no flow frame for >0.3s, `flow_hold_predict` freezes `pos_x_m/pos_y_m` and fast-decays `vx_est` instead of integrating IMU-only: the velocity leak (`IMU_LEAK` τ≈10s) amplifies a steady accel bias ~10× (v* = a·dt/(1−λ)). Trade-off: real drift during a flow outage goes unrecorded.
1d. **DC rejection on both IMU channels feeding the estimator (2026-07-04)** — dead-reckoned position ran meters-per-minute at rest even at 1m height with good qual. Two DC injection paths, both now high-passed with slow EMA bias trackers (states survive `flow_hold_reset` so takeoff protection is immediate):
   - gyro DC bias (temp drift / post-handling shift) → fake velocity `k×bias×h` through the metric comp term (1°/s at 1m = 0.017 m/s, past the deadband, scales with height) → comp uses `ω − ω_lp` (`GYRO_LP_ALPHA` τ≈1-2s);
   - **gravity leakage** `g·sinθ` from tilt (controlled A/B test: on ground post-calib accel≈0 & stable; hand-held at 1m accel≈0.5 m/s² [≈3° tilt] → constant vel offset → drift; a hover's 1-2° trim tilt does the same) → predict integrates only `a − a_lp` (`ACCEL_LP_ALPHA` τ≈1.5s).
   The IMU channel's job in the complementary filter is high-frequency transients only (flow provides absolute DC velocity), so HPF costs nothing by design. Verify at rest (any tilt): `Vel vx/vy` ≈ 0.00 and X/Y static.
1e. **Accel channel demoted to 0 — flow-only velocity (2026-07-04)** — after a fast push-and-stop, X kept growing: the accel DC tracker is recency-biased, the stop's sharp negative decel spike drags `ax_lp` negative, and once stationary the HPF outputs a positive fake acceleration for the ~1.5s re-convergence. Structural, not tunable: an HPF has zero DC gain, so impulse absorbed during a spike is always paid back during recovery. Since metric flow + the 0.02 m/s deadband already cover hover needs (sub-deadband drift rate is negligible; 20–100fps needs no IMU bridging), and this channel caused every DC/transient bug so far, `ACCEL_GAIN` is now 0.0 — `vx_est` is driven purely by the flow complementary correction. All machinery (DC trackers, HPF) is kept; restore by changing one constant once a proper world-frame accel rotation (EKF/mag milestone) exists. Gyro-comp signs kx=ky=+1 bench-verified clean the same day (post-DC-fix tilt test).
1f. **Flow UART pipeline: accumulate, don't sample (2026-07-04)** — the root cause of "same move, random magnitude" (X read ~8% of true displacement, varying run-to-run, plus post-stop creep and occasional wrong-sign blips): `uart_read_bytes` with a 256-byte request at 19200 baud hit its 100ms timeout every time, batching ~21 frames per parse burst, and `get_data`'s latest-frame-wins semantics silently discarded ~95% of the displacement counts (also delaying consumed motion by up to 100ms — the "creep after stop"). Fixed: 10ms UART timeout (≤2 frames/batch) + the driver accumulates `acc_x/acc_y` counts which `get_data` hands over and zeroes; `flow_hold_update` consumes the accumulated counts over the measured window (`FRAME_DT_MAX` widened to 0.2s) — exact regardless of batching. Note: the old pre-metric position feedback used the driver-side integral and was lossless; the metric refactor introduced this regression by deriving velocity from per-frame counts through the lossy channel.
1g. **Input filters must run continuously (2026-07-04)** — the accel EMAs `g_ax_filt/g_ay_filt` were zeroed every loop in the DISARMED and low-throttle branches, so the estimator's DC tracker anchored to a 0.3×-scaled baseline; the moment resets stopped (calib-mode ON, or throttle-up at takeoff) the input stepped to full scale → ~0.5 m/s fake velocity for the ~2s tracker re-convergence → tens of cm of phantom drift (bench-reproduced at calib-mode ON; the same transient hit every real takeoff). Fix: never reset input-conditioning filters — only integrator outputs (`vx_est`, `pos_*`) get reset. Expected behavior now: calib-mode ON shows `Vel` ≈ 0 immediately, no transient window. Note on noise floors: at 1m height one flow count = 0.12 m/s metric, so instantaneous `Vel` jitter at height is quantization — judge position stability, and do precision tests at ~0.5m where a count is worth half as much.
