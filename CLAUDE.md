...
## Current Status & TODO (last updated 2026-07-07)

✅ **No-fly gate CLEARED 2026-07-07** (was active 2026-07-06→07). The optical-flow regression had **two causes**, both from the 2026-07-06 remount: (1) the frame/claw intruded into the sensor FOV — a static textured object anchors PMW3901 flow to ~0 *at high qual*, so "high qual" did NOT rule out obstruction; hand-carry under-reported asymmetrically. Fixed by remounting clear of the FOV. (2) The module ended up 180° from its 2026-07-04 orientation → both flow axes sign-inverted (forward `flow.x`≈−0.8, right `flow.y`≈−1.2). Fixed in `pv3901l1.c parse_byte()` (`flow_x=-raw_y`, `flow_y=raw_x`). **Re-verified on hardware 2026-07-07: forward→`flow.x`>0, right→`flow.y`>0, cross-terms ≈0, gyro-comp tilt test still nulls (kx/ky unchanged).** Residual scale ~0.8/1.2 (avg ~1.0) judged hand-carry noise, not chased. Flight is unblocked; the standing next-flight caution is still to re-verify flow axes after **any** future remount (this whole episode was a skipped remount re-check).
...
**Current priority — shrink the ~30cm hover drift** (all-flow levers, no new hardware; no-fly gate cleared 2026-07-07, flight testing unblocked):
...
### Feature status — built & bench-verified unless marked **PARKED**; none flight-tested yet

(The metric-flow pipeline itself is flight-validated — see "Flight-verified" above. Its estimator design rules, bench-calibrated constants, and the safe bench-test mode now live in the **Estimator Design Rules** section below. Safety hardening → **Safety mechanisms**; loop timing → **Coding Conventions**.)

1. **BN-880 magnetometer driver + frontend cleanup (2026-07-05, ⏸️ PARKED — hardware)** — `drivers/bn880_mag.c` probes three chip variants (QMC5883L@0x0D, HMC5883L@0x1E, IST8310@0x0E; HMC data order is X,Z,Y big-endian vs QMC/IST X,Y,Z little-endian; IST is single-measurement mode, re-triggered each read). On total probe failure it scans the whole bus and logs every ACKing address (only 0x29/0x68 = compass unpowered/not on bus). Gauss output in **module axes — no body-frame alignment or hard/soft-iron calibration yet** (that's the Mahony 9-axis step). ⚠️ **Hardware unresolved & PARKED (user decision 2026-07-05)**: compass never ACKed (vendor says "QMC5883"); flight test showed yaw nearly constant → drift is not yaw-induced → mag priority dropped. When resumed: flash latest firmware via the native-USB port, read the boot-log bus scan — unknown address = new variant to support (QMC5883P is 0x2C); only 0x29/0x68 = module-side hardware fault (check power LED, feed VCC 5V, verify silk-vs-cable pin order, continuity SDA→GPIO9/SCL→GPIO8). Init is non-fatal (`mag.ok=0` in telemetry when absent); read at 20Hz in the telemetry block. Web UI simplified the same day: removed IMU-raw/PID panels, flow debug rows (fps/raw/comp/corr/lock-target), GyroComp+FlowScale inputs (values are firmware defaults now; recalibrate via WS `flow_comp` command), dpad nudge buttons, battery placeholder; telemetry JSON trimmed to match (accel/gyro/pid/alt.out/flow debug fields dropped).

2. **MG995 机械爪驱动 (2026-07-05, 台架标定完成)** — `drivers/servo_grip.c`: GPIO10, LEDC TIMER_1/CH4 @50Hz (电机占 TIMER_0/CH0-3), 500–2500µs↔0–180° (参数沿用用户已验证的 esp32s3-mg995-servo-web 库), 主循环 100Hz 限速逼近 120°/s (防电流尖峰+飞行反扭, 无独立任务)。**标定值 (2026-07-06 新爪体, 舵机标定模式实测): 0°=完全张开, 117°=齿条行程极限, 闭合预设 110° (留 7° 防抓物堵转) — `SERVO_GRIP_MAX_DEG=117` 是驱动层硬限位 (clamp 在驱动内部, 任何上层都越不过), commander 侧同步钳位, 前端滑条 0–117**。机械爪兼作**起落架**: 地面停放/上电默认完全张开 (OPEN=0°, init 即输出)。WS `grip` 命令经 setpoint 非阻塞下发 (任意模式可用), 安全复位**保留**爪角度 (防断连掉落已抓目标), 遥测 `grip` 字段 + 前端调试面板。**待办**: ⚠️ 硬件接线必须独立 5V BEC (堵转 1A+, 共轨会在闭爪瞬间拉复位主控)。**传感器布局 (2026-07-06 用户确认)**: TOF/光流已改装位置, 抓着目标**不被遮挡** (携带飞行全功能); P4 相机在爪末端, **抓取后必盲 → 携带阶段无视觉**, 投放导航只能靠航位推算 (pos 坐标系同一解锁周期内连续)+人工; 相机被挡=天然抓取成功信号 (协议 v1.1 拟加 TRACK_BLOCKED)。降落策略方向: 永远先投放再降落 (垃圾负载, 投放永远安全); 拟定"标记投放点"方案见 DESIGN Phase 6。最终目标 (S3+P4 视觉伺服抓取) 见 DESIGN.md Phase 6。

3. **S3↔P4 串口协议 v1 (定稿 2026-07-05; ✅ 协议层联调实测通过 2026-07-07 — 链路/CRC/双向收发真机验证)** — 唯一权威定义 `communication/p4link_protocol.h` (P4 工程持同一份拷贝), 设计文档 DESIGN.md §3.10, **P4 侧开发独立规格 `P4LINK_PROTOCOL.md`** (字节表/解析器状态机/P4 实现清单/联调步骤, 自包含无需读飞控代码)。要点: P4=纯传感器永不指挥飞行 / UART2 TX=GPIO17 RX=GPIO18 @115200 (引脚已预留) / 帧 `AA 55 type len payload crc16` (CCITT-FALSE, 两侧必须用头文件里的参考实现) / 仅两条消息: MSG_STATE S3→P4 50Hz (姿态/高度/模式/爪角), MSG_TARGET P4→S3 (机体系偏差 dx/dy 同 move_to 约定 + conf + track, 未锁定时 ≥10Hz 心跳) / ts_echo 回传免时钟同步测陈旧度 / S3 四道安全门: 限幅 ±1m, conf≥50, 陈旧度 ≤200ms, 链路超时 500ms→中止抓取回悬停 (绝不 DISARM) + 准静态门 |tilt|>5° 不采信。**联调状态 (2026-07-07)**: 协议收发全过 (链路在线、CRC 零错、S3↔P4 双向帧正常)——协议层零问题, S3 侧零改动对接成功。**当前阻塞在 P4 视觉模型**: 台架测试几乎检测不到目标 (P4 侧模型/量化问题, 已反馈 P4 开发者), 导致轴向验证 (CAM_MOUNT 符号, 见 P4LINK_PROTOCOL.md §5.2) 与端到端抓取无法进行, 待 P4 模型修复后继续。

4. **p4link 驱动 + 抓取任务状态机 (2026-07-05; p4link 驱动收发 2026-07-07 台架实测通过, 抓取状态机端到端仍未测)** — `communication/p4link.c`: UART2 (TX=17/RX=18) + `p4link_rx` 任务 (Core1 prio9) 喂解析状态机, take 语义交付原始测量; **安全门控在 main.c** (协议四道门 + 准静态门), 主循环 50Hz 广播 MSG_STATE (非阻塞 uart_tx_chars)。`control/grab_mission.c`: IDLE→ALIGN(P4 边降边修: 视觉修正限速 1s/次, **对准容差随高度放宽** max(8cm, 0.15×h) — 高空光流噪声大只做粗对准, ×3 次连续确认, 台阶 0.25m, 视觉地板 0.35m, 对准计时每级台阶重置 + 120s 任务总超时)→DESCEND(末段开环)→GRASP(**TOF ≤ sp->grab_tof_m 触发闭爪**, 默认 0.20m=TOF 落地安装读数, 前端滑条 0.10–0.40 可调)→ASCEND(回起始高度)→IDLE。**参数按 ~1.7m 工作巡航高度设计**(用户 2026-07-05 确认: 下降行程 ~1.5m, 斜坡本身 5s)。闭爪经 `commander_set_grip()` 写 setpoint 且 GRASP/ASCEND **每拍重写** (与 WS 解析整结构体赋值的窄竞态丢写 10ms 自愈); 全局中止 = 模式退出/油门<5%/TOF 失效, **中止绝不张爪**。前端"抓取任务"面板: 触发高度滑条 + **测试抓取按钮** (`grab_start test=1`: 无 P4, 跳过 ALIGN, 假设触发时位置精确, 下降不做视觉修正 — 用户 2026-07-05 指定的测试假设) + 中止按钮; 遥测 `grab:{st,p4,mk,dx,dy,cf,tk,rok,cer}` (2026-07-07 增 P4 原始测量 dx/dy/conf/track + 链路统计 rx/crc, 前端抓取面板可视, 免盯 P4 串口——经 `p4link_peek_target()` 非消费读取)。**待办**: 抓取状态机端到端未测——被 P4 模型检测失败阻塞 (ALIGN 需 LOCKED 测量驱动); 模型修好后先做轴向验证再实飞验证触发高度/时序, 调参回填 grab_mission.c 顶部常量。

5. **投放任务 + 标记投放点 (2026-07-06, 编译通过, ⚠️ 实飞未测)** — 主线方案落地 (用户认可: 永远先投放再降落, 垃圾负载投放永远安全)。`grab_mission` 扩展 GRAB_MISSION_GRAB/DROP 两种任务类型 + GOTO(5)/RELEASE(6) 状态: **就地投放** = 测试抓取镜像 (下降→TOF≤drop_tof_m 触发张爪→回升, 默认 0.30m 前端滑条 0.15–0.60); **返航投放** = 悬停筐上方按"标记投放点"存 flow 航位坐标 (需 qg≥0.05) → 抓取后 `drop_start ret=1` → GOTO 状态 move_to 返航 (到达判据: 距标记 <0.25m 且位置环 hold; 用户方向键干预后限速 1s 重发 move_to 自愈) → 下降张爪 → 回升。**标记仅同一解锁周期有效** — main.c 在两处 flow_hold_reset 旁联动 clear_mark (DISARMED 分支 + 低油门分支), 坐标系复位标记即失效。投放后爪=张开=起落架就绪可直接降落; ASCEND 仅抓取任务每拍重写闭合角, 投放任务不干预。前端: 投放点状态行 + 投放高度滑条 + 标记投放点/就地投放/返航投放三按钮; 遥测 `grab.mk`。**待办**: 实飞验证返航精度 (配大开口筐吸收 ~0.5m 航位误差); P4 上线后协议 v1.1 加 TRACK_BLOCKED (相机被挡=抓取成功信号)。

6. **Vz 常值偏移修复 (2026-07-06, 编译通过, ⚠️ 实飞待验证)** — 实飞观察: 上升中 Vz 读负、起飞漂移大难以安全进悬停, 用户怀疑位置环从未介入 — 代码证实: accel-Z 零偏从未校准 (审计老项, MPU6050 规格 ±0.1g) → `az_up` 常值假加速度 → vz 互补滤波稳态偏移 ≈ **0.5×零偏** (K=0.2, TOF 10Hz) → 起飞位置锁的 |vz|<0.15 门永远不过, `g_position_lock_pending` 挂死, ps 停在 wait。修复两层: ① `mpu6050.c` 初始+重校准均计算 Z 零偏 (avg − 9.80665) 并在 read 扣除; `gyro_calib` 以 WARN 打印 **"accel Z bias"** (地面确认零偏大小的唯一入口 — **DISARMED 下遥测 Vz 恒 0 是代码行为**, altitude_reset 每拍清零, 手持竖动测不出任何东西); ② `altitude.c` 新增 az_up 直流跟踪器 (`altitude_track_az`, tau=2s, 主循环每拍**全模式**调用; `altitude_reset` 不清零 — flow_hold 同款"输入调理滤波器连续运行"规则), vz 只积分高通分量, 吸收静态校准治不了的**飞行中电机振动整流直流下移**。验证清单: 烧录→gyro_calib 看 Z bias 数值; 实飞悬停 Vz 围绕 0 波动、ps 进入 lock(2)。若爬升段漂移仍大 → 下一杆是 POS_KP/FLOW_KP (roadmap 既有项)。2026-07-06 追加: Z 零偏经 `mpu6050_get_accel_z_bias()` 进遥测 `alt.azb` (免串口, 前端 TOF 面板显示, |zb|>0.3 变黄); vz 互补滤波拆出 `altitude_update_vz()` 供测试模式在 DISARMED 下推进 (电机停转看 Vz 响应手持提放)。**地面滑行测 flow/位置保持无效** (爪腿撑起+倾斜+电机蹭地, 净位移≠肉眼峰值); 干净测 flow_scale 必须用测试模式手持匀速移动已知距离。

7. **✅ 光流欠报回归 — 已解决 (2026-07-06 发现→2026-07-07 关闭): 根因=FOV 被机架遮挡(高 qual 掩盖) + 换装 180° 致两轴符号反; 让开 FOV + `parse_byte` 翻号(`flow_x=-raw_y`/`flow_y=raw_x`)已真机复验通过, 禁飞门撤除 — 详见顶部横幅** — 测试模式手持实测: 前移 1m → flow.x≈0.1m, 右移 1m → flow.y≈0.7m (各 ±10cm)。背景: 光流模块 2026-07-06 改了安装位置且**当前仅胶带固定、非常不稳**; 本文件既有规则"换装必须重核轴映射 (pv3901l1.c parse_byte 两行) + 手推验证"未执行。两轴欠报不等比 (0.1 vs 0.7) → 纯旋转装错解释不了, 需完整交叉项数据。**flow 复验通过前禁止实飞** (轴向错误 → 位置环正反馈放大漂移)。**待办 (螺丝固定后)**: 4 方向×2 读数矩阵 — 前/后/左/右各 1m, 每次记录 flow.x **和** flow.y 含符号, 每次测前重开测试模式清零, 恒高 ~1m/机体水平/慢速匀速, 过程中记 qual 是否 >80 + tof 读数 vs 卷尺实高。判读: 交叉项大 → 改 parse_byte 轴映射; 对角项等比偏低 → 查 TOF/光流安装高度差与 flow_scale。附: Z 零偏实测 −0.38~−0.48 m/s² (首次偏大 0.1 = 温漂, 例行校准放上电 1-2 分钟后), 与 vz 诊断定量吻合。

8. **舵机标定模式 + 新爪体角度标定完成 (2026-07-06)** — `{"cmd":"grip_calib","on":1}` / 前端机械爪面板"舵机标定"开关: **仅 DISARMED 生效** (main.c 每拍门控 `servo_grip_set_calib_mode(mode==DISARMED && sp->grip_calib)`, 解锁瞬间自动退出), 开启后驱动+commander 限位放开 0–180°, 前端滑条量程随之切换, 转速降 45°/s (慢速逼近, 超齿条行程打滑前来得及收手); 关闭/失联复位自动恢复硬限位, 超限目标慢速滑回。**标定结果已回填 servo_grip.h 三宏 (2026-07-06 实测): OPEN=0°, 行程极限 117° (=MAX_DEG), CLOSE=110° (留 7° 防抓物堵转过热)**。
...
- ~~accel-Z bias never calibrated~~ — **fixed 2026-07-06** (see status item 6)
...
**Runtime access**: the drone is a WiFi AP — SSID `Drone-XXXX` (XXXX = MAC suffix), password `12345678`, web UI at `http://192.168.4.1` (WebSocket commands + 20Hz telemetry). Boot log prints the exact SSID.
...
## Estimator Design Rules (learned from bench-reproduced bugs — do not regress)

**Bench-calibrated flow constants** (2026-07-04, now code defaults — still lost on reboot, no NVS yet, but defaults match bench values):
- `flow_scale = 0.00244` rad/count — verified by 1m hand-carry push (<10cm repeatability)
- gyro-comp `kx = ky = +1.0` — clean tilt test after the DC fixes below
- flow mount remap in `pv3901l1.c parse_byte()` — **re-verified 2026-07-07 after remount** (now 180° from the 2026-07-04 orientation): body-forward = -raw_y, body-right = raw_x
- accel axis mapping — IMU X points backward (negated), IMU Y points right (direct); verified by fast-push test

⚠️ **Re-verify all of these whenever the flow module is remounted** (this is exactly the no-fly gate open in Status — the 2026-07-06 remount skipped it).

**Rules** (each earned from a real bench-reproduced bug):
...
- **Input-conditioning filters run continuously** (accel EMA, DC trackers, `altitude_track_az`) — per-loop resets anchored the trackers to a scaled-down baseline, and every arm/calib transition produced a ~2s fake-velocity transient (~0.5 m/s, tens of cm of phantom drift — also hit every real takeoff).
...
**Bench-test mode / `flow_calib`** (`{"cmd":"flow_calib","on":1}`, front-end "测试模式" button): in DISARMED **motors stay stopped (props never spin — the DISARMED branch always calls `motor_stop`)** but **all estimators keep running** — hand-carry horizontally to calibrate `flow_scale` (watch `flow.x`), lift vertically to watch `alt.vz` respond (`altitude_update_vz` runs each tick). Auto-cleared on disconnect/timeout. This is the **only** safe way to validate estimators without arming (ALT_HOLD would spin props), and the harness for the no-fly-gate flow matrix. `alt.azb` (accel-Z bias) needs no mode — it's always in telemetry after `gyro_calib`. ⚠️ **Ground-taxi flow testing is invalid** (claw-legs tilt the frame + motors scrape ground; net displacement ≠ eyeballed peak) — clean `flow_scale` requires hand-carry over a known distance.
...
## C Headers — non-obvious contracts

All component headers are public (no `private_*.h`), each `#pragma once` + `extern "C"`. Function names and signatures live in the `.h` files; only the gotchas that aren't obvious from the signature are called out here.

- **`bn880_mag.h`** — `bn880_mag_init()` probes QMC5883L@0x0D → HMC5883L@0x1E → IST8310@0x0E and logs a full I2C bus scan if all three fail; **non-fatal**. `bn880_mag_read()` returns gauss in **module axes** (unaligned/uncalibrated until the Mahony 9-axis milestone).
- **`servo_grip.h`** — `servo_grip_set_angle()` enforces the driver-level hard limit `SERVO_GRIP_MAX_DEG=117°` (rack travel limit; *every* caller is clamped inside the driver). Presets `SERVO_GRIP_OPEN_DEG=0` / `SERVO_GRIP_CLOSE_DEG=110` (travel limit −7° to avoid stall). `servo_grip_update(dt)` slews at 120°/s (45°/s in calib mode). `servo_grip_set_calib_mode()` opens the limit to 0–180° but main.c only lets it engage while DISARMED. **Non-fatal** init; claw doubles as landing gear (opens on power-up).
- **`flow_hold.h`** — the **effective** gyro-comp default is **+1/+1**, but the source of truth is `commander.c`'s setpoint defaults (`flow_kx/ky/scale`) which the main loop pushes into flow_hold every tick — `flow_hold_init()`'s own zeros never take effect. `flow_hold_predict()` (every 100Hz tick: IMU integrates velocity + position, runs the velocity PID) is separate from `flow_hold_update()` (fresh flow frames only: gyro comp → metric conversion via TOF height → complementary correction, no PID). `flow_hold_reset()` keeps calibration (`gyro_kx/ky`, `flow_scale`).
- **`position.h`** — two modes distinguished by the `hold` flag: `position_set_target()` (`move_to`, one-shot, meters, transitions into hold on arrival) vs `position_hold_start()` (lock current point, persistent). `position_update()` debounces move_to arrival (10 consecutive in-tolerance ticks).
- **`grab_mission.h`** — mission layer over altitude/position/servo_grip/commander. `grab_mission_clear_mark()` **must** be called wherever `flow_hold_reset()` is (a coordinate-frame reset invalidates the stored drop mark — main.c already links both points). `grab_mission_abort()` keeps the claw angle (never drops a held target). P4 measurements (`grab_meas_t`) arrive **pre-gated from main.c**, so control never includes communication headers. States `grab_state_t` (IDLE/ALIGN/DESCEND/GRASP/ASCEND/GOTO/RELEASE = telemetry `grab.st` 0–6); `grab_mission_kind_t` (GRAB/DROP forks the DESCEND trigger height + claw action).
- **`p4link.h` / `p4link_protocol.h`** — `p4link_take_target()` has take semantics (each frame delivered once) and returns **raw** values; **all safety gating lives in main.c**, not the driver. `p4link_init()` is **non-fatal**. `p4link_protocol.h` is the sole authoritative protocol definition (frame format, packed structs, CRC-16/CCITT-FALSE reference impl) — the P4 project holds an identical copy; see `P4LINK_PROTOCOL.md` / DESIGN.md §3.10.

(The trivial leaf headers — `i2c_bus`, `mpu6050`, `tof400f`, `pv3901l1`, `motor`, `attitude`, `pid`, `mixer`, `commander`, `altitude`, `wifi_ap`, `http_server`, `web_page` — are self-explanatory from their signatures; the WebSocket command surface is documented under Key Design Decisions.)
