...
8. **p4link 驱动 + 抓取任务状态机 (2026-07-05, 编译通过, ⚠️ 台架/实飞未测)** — `communication/p4link.c`: UART2 (TX=17/RX=18) + `p4link_rx` 任务 (Core1 prio9) 喂解析状态机, take 语义交付原始测量; **安全门控在 main.c** (协议四道门 + 准静态门), 主循环 50Hz 广播 MSG_STATE (非阻塞 uart_tx_chars)。`control/grab_mission.c`: IDLE→ALIGN(P4 边降边修: 视觉修正限速 1s/次, 对准容差 8cm×3 次连续, 台阶 0.15m, 视觉地板 0.35m)→DESCEND(末段开环)→GRASP(**TOF ≤ sp->grab_tof_m 触发闭爪**, 默认 0.20m=TOF 落地安装读数, 前端滑条 0.10–0.40 可调)→ASCEND(回起始高度)→IDLE。闭爪经 `commander_set_grip()` 写 setpoint 且 GRASP/ASCEND **每拍重写** (与 WS 解析整结构体赋值的窄竞态丢写 10ms 自愈); 全局中止 = 模式退出/油门<5%/TOF 失效, **中止绝不张爪**。前端"抓取任务"面板: 触发高度滑条 + **测试抓取按钮** (`grab_start test=1`: 无 P4, 跳过 ALIGN, 假设触发时位置精确, 下降不做视觉修正 — 用户 2026-07-05 指定的测试假设) + 中止按钮; 遥测 `grab:{st,p4}`。**待办**: 实飞验证触发高度/时序, 调参回填 grab_mission.c 顶部常量。
...
- `grab_mission.h` — 抓取任务状态机: `grab_mission_init()`, `grab_mission_start()` (前置校验: ALT/POS_HOLD + TOF 有效且高于触发高度+10cm + 爪已张开 + 非测试需 P4 存活), `grab_mission_abort()` (回起始高度, **爪保持当前角**), `grab_mission_update()` (100Hz 推进; 内置全局中止: 模式退出/油门切断/TOF 失效), `grab_mission_active()`, `grab_state_t` (IDLE/ALIGN/DESCEND/GRASP/ASCEND — 遥测 `grab.st`), `grab_meas_t` (已过安全门的视觉测量, main.c 门控后传入)
