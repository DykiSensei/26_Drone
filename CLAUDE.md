...
⚠️ **NO-FLY GATE ACTIVE (2026-07-06)**: an unresolved optical-flow under-reporting regression (bench test-mode measured forward 1m → `flow.x`≈0.1m but right 1m → `flow.y`≈0.7m — the two axes under-report by *different* ratios). The flow re-verification matrix in **status item 7** below must pass before any flight; an axis error makes the position loop a positive-feedback fly-away. Do not recommend or plan flight tests until it clears.
