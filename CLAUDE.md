...
1b. **Flow mount remap + metric gyro comp (2026-07-04)** — module found mounted 90° rotated (forward→raw_y+, left→raw_x+); remapped to body frame in `pv3901l1.c parse_byte()`. Gyro comp moved from counts domain to metric domain (comp = k × ω × height; counts-domain constant k could not null once dt_frame became measured/jittering — tilt test swung at every k value). kx/ky are now dimensionless, nominal ±1.0, default −1.0/−1.0 — ⚠️ **sign not yet bench-verified: do the tilt test** (标定模式 on, hover by hand ~0.5m, rock gently ≤15° about one axis without translating, watch `Comp x/y`; if it swings with the rocking, flip that k's sign).
