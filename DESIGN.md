...
- [ ] 抓取后投放流程（**原则: 永远先投放再降落**——垃圾负载, 投放永远安全, 不走"抓着降落"路线。
      前提事实 2026-07-06: TOF/光流已改装位置携带时不被遮挡; P4 相机在爪末端**抓取后必盲**→携带
      阶段无视觉。拟定方案: ①DROP 任务=测试抓取镜像 (下降→张爪→回升); ②"标记投放点"=起飞后飞到
      筐上方记录 pos_x/y, 抓取后 move_to 航位推算返航 (同一解锁周期 pos 系连续); ③大开口筐吸收
      ~0.5m 航位误差; ④张爪后相机复明→视觉复查/搜索下一目标; ⑤相机被挡=抓取成功信号 (协议 v1.1
      拟加 TRACK_BLOCKED, 向后兼容)——方案待用户确认后实现)
