...
│   │   ├── servo_grip.h / .c   # MG995 机械爪舵机（0°张开/110°闭合/117°硬限位）✅
│   │   └── motor.h / .c        # 电机 PWM 驱动 ✅
│   ├── control/                # 飞行控制
│   │   ├── CMakeLists.txt
...
- [x] 机械爪台架标定（2026-07-06 新爪体重标定，舵机标定模式实测：**0°=完全张开, 117°=齿条行程极限=`SERVO_GRIP_MAX_DEG` 驱动层硬限位, 闭合预设 110°（留 7° 防抓物堵转过热）**；爪兼作**起落架**，地面/上电默认完全张开）
- [x] 抓取后投放流程（**原则: 永远先投放再降落**——垃圾负载, 投放永远安全, 不走"抓着降落"路线。
      前提事实 2026-07-06: TOF/光流已改装位置携带时不被遮挡; P4 相机在爪末端**抓取后必盲**→携带
      阶段无视觉, 投放导航=航位推算+人工。已实现 2026-07-06 (⚠️ 实飞未测): ①就地投放 = 测试抓取
      镜像 (下降→TOF≤投放高度张爪→回升, 默认 0.30m 可调); ②标记投放点 (悬停筐上方记录 pos_x/y,
      仅同一解锁周期有效, 坐标系复位自动失效) + 返航投放 (move_to 航位返航→到达 <0.25m→下降张爪);
      ③投放后爪张开=起落架就绪; 前端三按钮 + 投放高度滑条 + 遥测 grab.mk）
- [ ] 投放实飞验证（返航精度实测; 配大开口筐 40cm+ 吸收 ~0.5m 航位误差——环境侧工程化）
- [ ] 张爪后相机复明→视觉复查投放/搜索下一目标 (P4 上线后); 相机被挡=抓取成功信号 (协议 v1.1
      拟加 TRACK_BLOCKED, 向后兼容, S3 只判 ==LOCKED 不受影响)
